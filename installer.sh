...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/erlkim/hi/main/authbot > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/erlkim/hi/main/authbot | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/erlkim/hi/main/authbot | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
dircreate
        ...
        echo ""
        echo -ne "Your name store [dafult: ERLKIM-STORE] : "
        read store_pnl
        [[ -z $store_pnl ]] && store_pnl="ERLKIM-STORE"
        echo ""
...
EOF
        ...
        fun_bot1() {
            [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
				wget -qO- https://raw.githubusercontent.com/erlkim/hi/main/ShellBot.sh >/etc/.maAsiss/.Shellbtsss
			}
